...
echo "# SVHN+Extra hyper-parameters for all methods under comparison."
...
for size in 250 500 1000 2000 4000; do
    common_args="--train_dir experiments/compare --dataset=svhn.${seed}@${size}-1"
    echo "python ict.py $common_args --wd=0.02 --beta=0.5 --c_weight=20"
    echo "python mean_teacher.py $common_args --wd=0.02 --smoothing=0.001 --c_weight=180"
    echo "python mixmatch.py $common_args --wd=0.02 --beta=0.25 --w_match=250"
