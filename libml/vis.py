...
class FeatureSpace(object):
	def __init__(self, feature_list, label_list, filepath_list):
		self.feature_list = np.array(feature_list, dtype=np.float32)
		self.label_list = np.array(label_list, dtype=np.int32)
		self.filepath_list = filepath_list

	def concat(self, fs):
		self.feature_list = np.concatenate([self.feature_list, fs.feature_list], axis=0)
		self.label_list = np.concatenate([self.label_list, fs.label_list], axis=0)
		self.filepath_list += fs.filepath_list
	

class NearestNeighborPlotter(object):
	def __init__(self):
		pass
