...
class BaseMultiModel(ClassifySemi):
    ...
    def get_kernel_by_layer(self, layer_name):
        ...
        v_list = [v for v in self.weights_dict[layer_name]
                  if 'kernel' in v.name]
        assert len(v_list) == 1
        return v_list[0]
...
class CNN13(BaseMultiModel):
    ...
    def classifier(self, x, scales, filters, training, getter=None, **kwargs):
        ...
        conv_args = dict(
            kernel_size=3, activation=tf.nn.leaky_relu, padding='same')
...
class ConvNet(BaseMultiModel):
    ...
    def classifier(self, x, scales, filters, getter=None, **kwargs):
        ...
        for scale in range(scales):
            layer_func_dict['conv%d_1' % (
                scale+2)] = partial(tf.layers.conv2d, filters=filters << scale, **conv_args)
            layer_func_dict['conv%d_2' % (
                scale+2)] = partial(tf.layers.conv2d, filters=filters << (scale+1), **conv_args)
            layer_func_dict['avgpool%d' % (
                scale+2)] = partial(tf.layers.average_pooling2d, pool_size=2, strides=2)

        layer_func_dict['conv%d' % (scales+2)] = partial(
            tf.layers.conv2d, filters=self.nclass, kernel_size=3, padding='same')

        layer_func_dict['avgpool%d' %
                        (scales+2)] = partial(tf.reduce_mean, axis=[1, 2])
    ...
    def classifier(self, x, scales, filters, repeat, training, getter=None, **kwargs):
        ...
        def residual(x0, name, filters, stride=1, activate_before_residual=False):
            with tf.variable_scope(name):
                ...
                x = tf.layers.conv2d(
                    x, filters, 3, strides=stride, **conv_args(3, filters))
                ...
                if x0.get_shape()[3] != filters:
                    x0 = tf.layers.conv2d(
                        x0, filters, 1, strides=stride, **conv_args(1, filters))

                return x0 + x

        layer_func_dict = OrderedDict([
            ('inp', lambda x:(x-self.dataset.mean)/self.dataset.std),
            ('conv1', partial(tf.layers.conv2d, filters=16,
                              kernel_size=3, **conv_args(3, 16))),
        ])

        for scale in range(scales):
            layer_name = 'res%da' % (scale+1)
            layer_func_dict[layer_name] = partial(residual, name=layer_name, filters=filters << scale, stride=(
                2 if scale else 1), activate_before_residual=(scale == 0))
            for i in range(repeat-1):
                layer_name = 'res%d%s' % (scale+1, chr(ord('b')+i))
                layer_func_dict[layer_name] = partial(
                    residual, name=layer_name, filters=filters << scale)

        layer_func_dict['bn'] = partial(
            tf.layers.batch_normalization, **bn_args)
        layer_func_dict['act'] = leaky_relu
        layer_func_dict['avgpool'] = partial(tf.reduce_mean, axis=[1, 2])

        layer_func_dict['fc1'] = partial(
            tf.layers.dense, units=self.nclass, kernel_initializer=tf.glorot_normal_initializer())
        layer_func_dict['logits'] = tf.identity
...
class ResNet18(BaseMultiModel):
    ...
    def classifier(self, x, scales, filters, repeat, training, getter=None, **kwargs):
        ...
        layer_func_dict = OrderedDict([
            ('inp', lambda x:(x-self.dataset.mean)/self.dataset.std),
            ('conv1', partial(tf.layers.conv2d, filters=64,
                              kernel_size=7, strides=2, **conv_args(7, 64))),
...
class ShakeNet(BaseMultiModel):
    ...
    def classifier(self, x, scales, filters, repeat, training, getter=None, **kwargs):
        ...
        def residual(x0, name, filters, stride=1):
            with tf.variable_scope(name):
                def branch():
                    ...
                    x = tf.layers.conv2d(
                        x, filters, 3, **conv_args(3, filters))
                ...
                if stride == 2:
                    x1 = tf.layers.conv2d(tf.nn.relu(
                        x0[:, ::2, ::2]), filters >> 1, 1, **conv_args(1, filters >> 1))
                    x2 = tf.layers.conv2d(tf.nn.relu(
                        x0[:, 1::2, 1::2]), filters >> 1, 1, **conv_args(1, filters >> 1))
                    x0 = tf.concat([x1, x2], axis=3)
                    x0 = tf.layers.batch_normalization(x0, **bn_args)
                elif x0.get_shape()[3] != filters:
                    x0 = tf.layers.conv2d(
                        x0, filters, 1, **conv_args(1, filters))
                    x0 = tf.layers.batch_normalization(x0, **bn_args)
                return x0 + x
        ...
        for scale, i in itertools.product(range(scales), range(repeat)):
            ...
            if i == 1:
                layer_func_dict[layer_name] = partial(
                    residual, name=layer_name, filters=filters << scale, stride=2 if scale else 1)
            else:
                layer_func_dict[layer_name] = partial(
                    residual, name=layer_name, filters=filters << scale)
...
class MultiModel(CNN13, ConvNet, ResNet, ShakeNet, ResNet18):
    ...
    def get_model(self, x, arch, getter=None, **kwargs):
        if arch == self.MODEL_CNN13:
            ...
            raise ValueError(
                'Model %s does not exists, available ones are %s' % (arch, self.MODELS))
    ...
    def classifier(self, x, arch, getter=None, verbose=False, **kwargs):
        ...
        if not arch.startswith('bcnn'):
            ...
            with tf.variable_scope(arch[0].upper()+arch[1:].lower(), reuse=tf.AUTO_REUSE, custom_getter=getter):
                ...
                layer_func_dict = self.get_model(
                    x, branch_arch, getter=getter, **kwargs)
                ...
                with tf.variable_scope('branch1', reuse=tf.AUTO_REUSE, custom_getter=getter):
                    out1 = out2 = self.apply(
                        x, layer_func_dict, verbose=verbose, stop_at=cnn_out_layer)

                if len(arch.split('_')[1:]) == 2:

                    branch_arch2 = arch.split('_')[2]
                    layer_func_dict2 = self.get_model(
                        x, branch_arch2, getter=getter, **kwargs)
                    cnn_out_layer2 = self.get_conv_out_layer(
                        branch_arch2, **kwargs)
                    ...
                    with tf.variable_scope('branch2', reuse=tf.AUTO_REUSE, custom_getter=getter):
                        out2 = self.apply(
                            x, layer_func_dict2, verbose=verbose, stop_at=cnn_out_layer2)
...
flags.DEFINE_enum('arch', MultiModel.MODEL_RESNET, MultiModel.MODELS + tuple(
    ['bcnn_%s' % m for m in MultiModel.MODELS]) + tuple(['bcnn_%s_%s' % (m, m) for m in MultiModel.MODELS]), 'Architecture.')
